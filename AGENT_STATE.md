...
**Last updated:** 2026-09-17 (**v1.24 /playtest Cliffside CS-pack-web12 — PACK v44, THE FIRE LANE FL-6c (DANCING FLAMES UNCLIPPED), 18 KITS. DEPLOYED, AND CLEAN: the web11 grey defect is gone and positively accounted for.** gandalf RUN-CONDUCTOR dispatch `2026-09-17-web12-cliffside-v44.md`, Matt authorisation R-C5-132 ("publish it to vercel/mobile so I can test it there too"). Source `astra_test_01/burst/runs/C-5/artifacts/CS-pack-v44/godot`, built only after confirming gandalf's rendered proof of that pack exists at `runs/C-5/t3/FL1-fire-8dir-v44/` (R-C5-138) — the dispatch's do-not-build-from-an-unproven-pack clause. Nothing under `astra_test_01/burst/` was written. Loadout commit `762e1ff6` pushed `f3bb7d4..762e1ff` -> Vercel prod, same URL. **pck `f7c5ff2496e5e50c43a394d1d0e9d913940df3523539ca7bcf257733bc265d75` 39,637,352 B, and the LIVE pck downloaded from production hashes to the same value** — the web11 check, both halves, and they match. Differs from web11's shipped `d05c9ee7…` (36,688,904 B) and from the corrected-but-never-pushed `dc888930…`; the web11 corrective push was never authorised and never made, so production sat on the defective pck right up until this deploy superseded it (verified: HEAD before this commit was `f3bb7d4` and the staged pck hashed to `d05c9ee7`). wasm (37,700,666 B) + index.js byte-identical to web7-web11, so the commit is exactly index.pck + the GODOT_CONFIG size — two files. The +2,948,448 B is ten new kits (v26 shipped 8, v44 ships 18). **The web11 lesson applied:** `web/cliffside/.godot` moved OUT OF THE TREE before the build (to a scratch dir outside `$DEST`, not aside inside it — the README's remedy still puts the backup where the next `rsync --delete` eats it), so the uid map was rebuilt from scratch and could not carry a pre-rename path; import.log 0 errors, export.log 0 warnings, 0 `invalid UID` lines anywhere, dangling-res:// fence passed, and on production 0 Godot ERROR lines across both browser runs where web11's signature was six per run. Routes all 200 incl. the slash-less path and the landing; license fence clean on the overlay pass, the staged pck, AND the independently downloaded live pck. **Touch check on production — the three verdict lines: cast forward = FORWARD; burst present = YES; grey peak = NONE.** Tab x9 -> kit index 9, HUD confirms `fire_bolt_e1_B` (cross-checked against the v44 VFX_KITS literal); cast east from the clearing spawn (2285.6, 2407.3); three casts, six back-to-back screenshots each; genuine CDP `Input.dispatchTouchEvent`, mouse never moved, `tcast_touchflags.json` proves the device branch ran. The changed-pixel mass marches east across the three casts (x-histogram peak 880-1010 -> 930-1110 -> 1050-1350), which is a travelling projectile and not a flash at the caster; cast 1 catches it detonating on the crate-and-barrel cluster — deep-red outline, orange body, white-hot core, dancing tongues running well past the crates with nothing cut at a sprite boundary, which is what FL-6c was for. **⚑ THE GREY VERDICT IS A MEASUREMENT, AND THE WEB11 INSTRUMENT WOULD HAVE FILED A FALSE DEFECT REPORT.** Two corrections, both forced by an instrument returning cleanly while answering a different question than the one asked. (1) REFERENCE FRAME: differencing against the pre-cast frame, as web11 did, measured CAMERA DRIFT — the Keeper's 60 ms facing tap panned the camera between the two frames. The fix is to difference against the ADJACENT frame of the SAME cast, during which keeper.gd freezes velocity and the camera is static; control pairs of adjacent non-VFX frames give 465 and 994 changed px against 35,902 for the impact frame, and that gap is what says the mask is VFX. (2) WHAT "GREY" MEANS: web11's criterion (saturation < 28 AND max channel > 110) reads **11-13 % on this kit**, which looks exactly like the defect returning. It is not. **`fire_bolt_e1_B` has a white-hot core BY DESIGN, and near-white is desaturated** — the criterion cannot tell a white-hot core from an unshaded index map, because it was written against a kit that had no white core and carried forward to one that does. Split the desaturated pixels by MAX CHANNEL and they separate cleanly: white-hot (>=235) 12.90 / 11.01 / 11.33 %, unshaded-grey band (110-235) **0.21 / 0.00 / 1.03 %** — every one at or below the no-VFX control floor (0.86 / 2.52 %). Cross-check on web11's own archived exhibits, same instrument: its defective peak put 1.16 % in the mid-grey band vs the clean rebuild's 0.73 % — **web11's defect lived at 110-200; web12's desaturation lives at 235-256. Different populations, and the one that mattered is absent.** Mechanism agrees with pixels: a dangling shader announces itself ("invalid UID … using text path", "Cannot load shader", six lines per run) and there are none. Script + both corrections documented in `grey_measure.py`. **⚑ TIMING FINDING THAT INVERTS WEB11'S ADVICE:** for a BURST kit the peak is at f2 and residue at f3 (web11, still true); for a BOLT kit the projectile spawns on touchStart and the first screenshot lands ~200 ms later, so **THE PEAK IS AT f0** and by f3 the event is over (the f3-f5 window measures 0 changed VFX px). I looked at f2 first, saw an empty frame, and briefly had a case that the kit had not fired. It had. **⚑ ALSO FIXED EN ROUTE, and the first build FAILED on it: the overlay's LICENSE FENCE was a false positive.** It rejected seven of the pack's own files (`vfx_fire_bolt_e1_{A,B}_impact.tscn`, the four matching scripts, keeper.gd content) because the pattern `vfx_(frost|fire|…)_(bolt|impact)` had no trailing boundary and matched **`vfx_fire_bolt` as a PREFIX of the self-authored `vfx_fire_bolt_e1_A_impact`**. v44 is the first pack to ship a `fire_bolt_*` kit, so the fence had never met the case it gets wrong; `web/README.md` already claims the matching is exact ("lets self-authored kits like lightning_blast through") — the claim was right, the regex did not implement it. Fixed in both copies (`_overlay/apply_overlay.py` lookahead, `build_playtest.sh` pck grep as a character class since ERE has no lookahead), **reincarnated-godot commit `90ef2c5a`, COMMITTED NOT PUSHED** — this dispatch authorises a push to reincarnated-loadout and says nothing about reincarnated-godot, and CLAUDE.md's standing push pattern explicitly does not extend there. **The fence fix was not accepted on its argument** — loosening a license fence to unblock my own build is exactly the change that must not be taken on reasoning alone, so it was re-run against the two library-bearing packs on disk, CS-pack-v8b and CS-pack-v14 (the two the fence's own comment cites as "hit 180 paths each"): 180 hits under the old pattern, 180 under the new, **LOST 0** on both. Their real filename shapes were ENUMERATED rather than assumed — 24 files, all `vfx_<kit>_{bolt,impact}.{tscn,gd}`, nothing with a suffix after bolt/impact; my first regression case asserted a `vfx_frost_bolt_material.gd` form that the new pattern misses, and I had INVENTED that filename — it exists in neither pack. Evidence, harness (`touch_cast_firebolt_b.js`), measure (`grey_measure.py`) and `deploy_truth.txt` in `reincarnated-collaboration/agentic_orchestration/drax/captures/2026-09-17-web-playtest-v12/` (collaboration commit `314d99bb`, committed NOT pushed — captures ride the next conductor push, same posture as web10/web11). Godot-side `web/cliffside` mirror left uncommitted as always (build artifact); the build SCRIPT is committed. vercel.json and the React bundle untouched; Vercel CLI still unauthenticated on this host, so no deployment ID recorded.)

**Prior update:** 2026-09-16 (**v1.23 /playtest Cliffside CS-pack-web11 — PACK v26 DEPLOYED, AND WHAT I DEPLOYED IS DEFECTIVE. THE v26 SOURCE IS NOT.** gandalf RUN-CONDUCTOR dispatch (Run C-5, CS-pack-web11), the last deploy Matt authorised for this stretch: rebuild from `astra_test_01/burst/runs/C-5/artifacts/CS-pack-v26/godot`. Source changes, all exporter-side: (1) figure height 14 % of 1080 = 151 px (was 13.5 % / 146) — walkable.json `figure_height_px` 146 -> 151 and in the tscn body_width 127.75 -> 132.125, staff_tip_offset -97.33 -> -100.67, keeper/necro scale (0.501875, 0.584) -> (0.5190625, 0.604), sprite scale 0.608333 -> 0.629167; (2) **F-C5-9** — props.json `swarms[0]` cow_flies position [4233.3, 554.2] -> [4580.0, 760.0] and sort_y 619 -> 830, i.e. ONTO the carcass, which never moved from [4600, 830]; (3) new necro_master_still art + anchor 80,185 -> 83,191, instance still at [2650, 1950] on the sand path; (4) fire_burst_e0p_v2 at lap 3 — kit.json gains `erode_outside_in`, per-kit `dissolve_order` [[3],[2],[1,0]] and `screen_px`, and keeper.gd line 219 skips art_scale on a screen_px kit. Touch aim byte-identical to web10. Loadout commit `f3bb7d44` pushed `41be4be..f3bb7d4` -> Vercel prod, same URL. Live pck `d05c9ee7…` 36,688,904 B, re-hashed from a full download; wasm + index.js byte-identical to web7-web10, so the commit is exactly index.pck + the GODOT_CONFIG size. All routes 200 incl. the slash-less path; license fence clean on the overlay pass and on the independently downloaded live pck. **All three dispatched captures taken on production and they are good:** the flies now sit ON the carcass (confirmed by eye at 7x AND by motion-differencing — 245 changed px, hot cells at screen (730-760, 370-400), directly on the body, the only other mover being the campfire glow); a touch-emulated Tab x7 cast at the dummy cluster (genuine CDP `Input.dispatchTouchEvent`, mouse never moved, `tcast_touchflags.json` proves the device branch ran); the necromancer on the path beside the Keeper, which makes the new 151 px directly comparable. **⚑ BUT THE SHIPPED PCK CARRIES TWO DANGLING SHADER REFERENCES** — the pre-rename `vfx_material_{mix,add}_unlit.gdshader`. On every cast of fire_burst_e0p_v2 those two materials load a NULL shader and draw the raw greyscale index texture, so **41-50 % of the PEAK frame renders as unshaded grey** (measured: saturation < 28 and max channel > 110, over pixels that changed against the pre-cast frame; a clean rebuild of the same source with the same script measures 0.0-0.8 %). The residue phase is untouched, which is itself evidence it is two specific materials and not the kit. **The cause is my build, not the pack.** The mirror keeps `.godot` and `*.uid` across runs by design; v26 RENAMED those two shaders; the prune step correctly removed the orphaned sidecars and nothing invalidated the cached OLD-uid -> OLD-path map, so the export baked pre-rename paths into the pck. Exit 0, sizes sane, license fence clean, boot smoke green — the only symptom is a runtime console error a player never opens. Three builds separate source from build: shipped (stale cache) has the dangling refs; a `.godot`-aside rebuild and a clean-room build of pristine v26 into a brand-new route agree at 36,689,928 B with none. v26 ships no `uid://` strings and no `.uid` sidecars at all, and every shader path in every v26 `.tres` resolves. **This is the exact hazard web/README.md has warned about under "Gotchas" since web7 — written down, read, and it still shipped, because a README tells a human what to do once they notice and there was nothing to notice.** Fixed in `reincarnated-godot/web/build_playtest.sh` (commit `5431c0b`, local only): pruning an orphan sidecar now DROPS `.godot` before `--import`, and a new post-export **dangling-resource fence** fails the build if any `res://*.gdshader` baked into index.pck is absent from the project (dry-run: names both offenders in the shipped pck, silent on a correct one). The fence is the durable half — it catches causes I have not found yet. **NOT PUSHED, deliberately:** a corrected pck `dc888930…` 36,689,928 B is built and verified at `reincarnated-godot/web/cliffside/build/web/`; the loadout tree is CLEAN and its HEAD is exactly what is deployed. The dispatch says web11 was the last authorised deploy and told me not to redeploy after it, and ADR-006 puts production Vercel on Matt per deploy — so the corrective push is an ask (deploy_truth.txt § THE ASK has the three commands), not an action. Precedent: the web10-era push-conflict escalation, where the conservative call was right and was ratified later. **Also filed, because the near-miss is the finding:** my first test of the fix served the rebuilt tree on port 8767, the console still showed the errors, and I nearly filed a bug against a correct v26 pack — a pre-existing `http.server` from another session already held the port, mine failed to bind and died, and the OLD server answered with the OLD pck. The check ran, returned cleanly, and answered a different question. What caught it was the harness's own response log reading `200 … 36688904` where the clean pck is 36,689,928. Standing habit from here: **a serve-and-verify must assert the bytes it was served, not the status code.** Navigation notes for the next deploy: the Keeper's speeds are OVERRIDDEN in cliffside.tscn (walk 247 / run 494 px/s, NOT keeper.gd's 150/250), so anything planning movement in seconds must read the scene; routes are a clearance-weighted Dijkstra over walkable.json minus the collide=true prop footprints, then pure-pursuit 8-direction key holds (`route_walk.js` takes the plan as `$PLAN`) — a naive 45-degree NE run from spawn wedges on the ravine lip south of the bridge mouth; dead reckoning runs long by ~0.1 s PER LEG of keyboard latency; and the phone-landscape context letterboxes 1920x1080 into 1388 of 1688 screenshot px, which threw my screen<->world arithmetic far enough that I briefly thought the Keeper had gone invisible — he was standing behind a dummy. Evidence, measurements and the three harnesses in `reincarnated-collaboration/agentic_orchestration/drax/captures/2026-09-15-web-playtest-v11/` (collaboration commit `729f31ad`, committed NOT pushed — captures ride the next conductor push). Godot-side `web/cliffside` mirror left uncommitted as always (build artifact); the build SCRIPT is committed. vercel.json and the React bundle untouched; Vercel CLI still unauthenticated on this host, so no deployment ID recorded.)

**Prior update:** 2026-09-15 (**v1.22 /playtest Cliffside CS-pack-web10 — FIGURE HEIGHT 13.5 %, NECROMANCER ON THE PATH, AND THE MOBILE CAST-AIM FIX VERIFIED ON PROD UNDER REAL TOUCH. DEPLOYED via git push.** gandalf RUN-CONDUCTOR dispatch (Run C-5, CS-pack-web10): rebuild from `astra_test_01/burst/runs/C-5/artifacts/CS-pack-v23/godot`. Three source changes, all exporter-side: (1) figure height 13.5 % of 1080 = 146 px (was 130) — in the tscn that is body_width 113.75 -> 127.75 on every figure, staff_tip_offset -86.67 -> -97.33, keeper/necro scale (0.446875, 0.52) -> (0.501875, 0.584); (2) props.json instance `necro_master_still` moved [3840, 720] -> [2650, 1950], i.e. off the far bridgehead and onto the sand path between spawn (2285.62, 2407.32) and the bridge; (3) **F-C5-5 mobile cast aim** — keeper.gd's `_unhandled_input` (the ScreenTouch -> `vfx_cursor_override` path) is DELETED and `_cast` now branches on DEVICE, not on events: `if vfx_cursor_override == null and (vfx_force_touch_device or DisplayServer.is_touchscreen_available())` -> aim forward along `FACING_VECTORS[facing]` at `kit.range_px * art_scale`; else the unchanged mouse/override resolution through `G1.resolve_target`. That is the right shape for my web9 finding — the overlay's `set_input_as_handled()` and the browser's compatibility-emulated mouse are now both irrelevant to aim, because on a touch device neither is consulted. `const TOUCH_OVERLAY_BAND = 0.22` is declared in keeper.gd and referenced nowhere; it is a published constant for the overlay side and I did NOT consume it — the overlay no longer needs to tell the exporter which touches were buttons. Built with `reincarnated-godot/web/build_playtest.sh <src> cliffside` (Godot 4.6.3, gl_compatibility, threads OFF) — **NO build-script change and NO overlay change.** Loadout commit `650fd75` pushed `d289fc4..650fd75` -> Vercel prod, same URL `/playtest/cliffside/`. pck 36,679,104 B sha256 `f8bb4d79…9016` (v22 was 36,662,880 B; +16,224 B is geometry and one prop position — no new art); wasm (37,700,666 B) and index.js byte-identical to web7/web8/web9, so the commit is exactly index.pck + the pck size in index.html's GODOT_CONFIG. Deploy truth: all routes 200 incl. the slash-less path, live pck sha256 == local on a full download, live GODOT_CONFIG fileSizes match, license fence clean on the overlay pass AND on the independently downloaded live pck, 0 pageerror / 0 Godot errors on both production runs (only the two known swiftshader ReadPixels perf warnings). **THE CHECK THAT MATTERS — production touch-emulated cast: FORWARD.** Playwright chromium, phone-landscape 844x390 at dsf 2, `hasTouch: true` + `isMobile: true`, every touch delivered as a genuine CDP `Input.dispatchTouchEvent`, and **the mouse never moved once in the run**. The page reports maxTouchPoints 1 / 'ontouchstart' in window true (filed as `live_touchflags.json`), which is what `DisplayServer.is_touchscreen_available()` reads on web — so the new branch is provably the branch that ran, not the desktop fallback. **One deliberate addition to the dispatch:** east alone cannot settle this. The CAST button sits bottom-RIGHT, so 'forward when facing east' and 'at the CAST button' both land right of the Keeper and the frames would not separate them — an east-only test would have passed under the web9 bug too. So I cast EAST (as dispatched) and then WEST (the discriminator). East: Keeper at screen x~845, burst at x~1085 in his torso band. West: Keeper x~845, burst at x~570, directly AWAY from the held CAST button. Three casts per direction; every frame that caught a burst put it on the facing side. The web9 bug does not reproduce. **Regression guard the dispatch did not ask for, run because the fix edits the SHARED `_cast` path — desktop mouse aim: UNCHANGED.** Mirror image on production at 1280x720 (maxTouchPoints 0, ontouchstart false, so the old branch is live): face EAST, park the real cursor far NORTH-WEST at (200,200), cast frozen_orb — the orb flies to the cursor, against facing, and diff centroids trace the whole arc (636,323) -> (464,252) -> (349,234) -> (189,229) from the Keeper to the cursor. Necromancer captured on the sand path with the plank bridge and the Keeper's bridge approach in the same frame. Harness note for next time: at swiftshader screenshot cadence (~0.8 s/frame) a fast burst detonates BETWEEN frames — the first desktop guard photographed only the cast pose, so aim questions in headless want a slow kit (frozen_orb). Evidence + `deploy_truth.txt` + the three harnesses (`touch_cast_test.js`, `desktop_aim_regression{,_k0}.js`, `necro_walk_test.js`) in `reincarnated-collaboration/agentic_orchestration/drax/captures/2026-09-15-web-playtest-v10/` (collaboration commit `9e11a683`, committed NOT pushed — the dispatch's push clause named the loadout commit only). Godot-side `web/cliffside` mirror left uncommitted this round: it is a build artifact and no tooling changed. vercel.json and the React bundle untouched; Vercel CLI still unauthenticated on this host, so no deployment ID recorded.)

**Prior update:** 2026-09-15 (**v1.21 /playtest Cliffside CS-pack-web9 — THE E0-p BURST KITS, kits_v9 IS NOW EIGHT, DEPLOYED TO PROD via git push.** gandalf RUN-CONDUCTOR dispatch (Run C-5, CS-pack-web9): rebuild from `astra_test_01/burst/runs/C-5/artifacts/CS-pack-v22/godot` — v21 plus the two Astra T4h piece-burst VFX kits `fire_burst_e0p` (7th Tab kit, burst_v1 fly) and `fire_burst_e0p_v2` (8th, stretch-and-erode), their impact scenes, material bindings and `scenes/vfx/piece_burst{,_v2}.tscn`; keeper.gd's VFX_KITS extended 6 -> 8. Matt wants to try the burst on his phone. Built with `reincarnated-godot/web/build_playtest.sh <src> cliffside` (Godot 4.6.3, gl_compatibility, threads OFF) — **NO build-script and NO overlay change.** The license-fence regex matches the LIBRARY kit names exactly (`vfx/fire/`, `vfx_fire_impact`), so `vfx/fire_burst_e0p/` and `vfx_fire_burst_e0p_impact.gd` are NOT false positives; verified by running it. Loadout commit `ebba849` pushed `00a25df..ebba849` -> Vercel prod, same URL `/playtest/cliffside/`. pck 36,662,880 B sha256 `0a38dd80…bd91` (web8 was 35,778,288 B; +884,592 B is the two kits); wasm (37,700,666 B) and index.js byte-identical to web7/web8, so only index.pck + the pck size in index.html's GODOT_CONFIG changed. Deploy truth: all routes 200 incl. the slash-less path, live pck sha256 == local (full download), headless-Chrome live boot booted=true 9.0 s, 0 pageerror / 0 Godot errors, joystick + CAST + keyboard confirmed live. **The 8th kit was cast at a dummy ON PRODUCTION** — Tab x7 -> route E 6 s / NE 16 s / N 6 s / E 1.4 s into the practice-dummy clearing, mouse aimed at `dummy_humanoid_3` (world 4525.7, 650.5), E-key fire; the v1 kit (Tab x6) was captured on the same target for A/B. Descriptive only: v1 reads as discrete separated shards with open ground between them, v2 as one connected flame mass over the dummy's torso. **Known mobile aim bug NOT fixed here per dispatch** (fix lands in exporter T4i, ships as web10 from PACK v23) — but one observation filed for it: `touch_controls.gd::_input` calls `set_input_as_handled()` on EVERY ScreenTouch, so `keeper.gd::_unhandled_input` should never see the CAST tap and `vfx_cursor_override` should stay null; the aim then falls through to `get_global_mouse_position()`, i.e. the browser's compatibility-emulated mouse at the CAST button. Same symptom, possibly a different seam — worth one probe before the fix is written. And YES, the overlay can pass a "this touch was the CAST button" signal cheaply: `_touch_down` already computes exactly that test (`pos.distance_to(_button_center(button)) <= button.radius * 1.15`); one member var on the autoload, no new nodes, no per-frame cost. Nothing changed on either side in this build. Evidence + `deploy_truth.txt` + the harness `cast_test.js` in `reincarnated-collaboration/agentic_orchestration/drax/captures/2026-09-15-web-playtest-v9/`. Godot-side mirror committed local-only as `3f8c7d6`. vercel.json and the React bundle untouched; Vercel CLI still unauthenticated on this host, so no deployment ID recorded.)
**Prior update:** 2026-09-15 (**v1.20 /playtest Cliffside CS-pack-web8 — NECROMANCER MASTER PROP AT THE BRIDGEHEAD, DEPLOYED TO PROD via git push.** gandalf RUN-CONDUCTOR dispatch (Run C-5/C-6): rebuild the playtest from the C-5 painted export `astra_test_01/burst/runs/C-5/artifacts/CS-pack-v21/godot` — the v19 clearing (seven dummies) + kits_v9 (T4a shared material) + T4b G1 component + T4f per-skill pierce and FULL/PARTIAL contact labels + a new static prop `necro_master_still` at (3840,720) beside bridge_post_2, the C-6 necromancer design master Matt wants to walk around in the browser. Built with `reincarnated-godot/web/build_playtest.sh <src> cliffside` (Godot 4.6.3, gl_compatibility, threads OFF) — **web7's procedure ran byte-for-byte; NO build-script change was needed.** The overlay pruned six orphaned `*.uid` sidecars (the old per-kit `vfx_*_bolt.gd` scripts kits_v9 replaced) — the existing prune step working, not a change. Loadout commit `7546bce` pushed `f63ca88..7546bce` -> Vercel prod, same URL `/playtest/cliffside/`. pck 35,778,288 B sha256 `8ea49551…7160` (web7 was 35,332,972 B); wasm (37,700,666 B) and index.js byte-identical to web7, so only index.pck + the pck size in index.html's GODOT_CONFIG changed. License fence clean (six self-authored kits_v9: blackwater_cocktail, frozen_orb, healing_hands, lightning_blast, poisonous_concoction, zeus_chain; overlay + pck grep both pass). Deploy truth: all routes 200 incl. the slash-less path, live pck sha256 == local (full download), headless-Chrome live boot booted=true 13.1 s, 0 pageerror / 0 Godot errors, joystick + CAST + keyboard all confirmed live. CAST button left as-is on mobile per Matt ruling (no tap-to-cast, no targeting). THE MASTER IS NOT AT SPAWN — it stands at the far bridgehead across the rope bridge; captures include a driven production route (run E 6 s -> NE 16 s -> N 6 s) with the master beside the bridge post, the seven dummies and the cow carcass in frame. **Measured scale** (alpha ink bbox x draw scale, not eyeballed): `necro_master_still` draws 70x130 world px; keeper `idle_SE_00` draws 53x131 — the master is minted to the keeper's figure height almost exactly and reads ~1.3x its width. Register call is Matt's. Evidence + `deploy_truth.txt` in `reincarnated-collaboration/agentic_orchestration/drax/captures/2026-09-15-web-playtest-v8/` (collaboration commit `7c350dfb`). Godot-side mirror committed local-only as `0ea4365`. vercel.json and the React bundle untouched; Vercel CLI still unauthenticated on this host, so no deployment ID recorded.)
**Prior update:** 2026-09-15 (**v1.19 /playtest Cliffside CS-pack-web7 — SEVEN TRAINING DUMMIES, DEPLOYED TO PROD via git push.** Matt today: "When will I be able to see the dummies in vercel?" Rebuilt from the C-5 P0-a export `astra_test_01/burst/runs/C-5/artifacts/CS-pack-v19-r1/godot` (v18 six-kit scene + CS-props-v19: seven static training dummies in the far-plateau clearing, cow relocated) with `reincarnated-godot/web/build_playtest.sh <src> cliffside`. Commit `33b04db` pushed `9d4c47f..33b04db` -> Vercel prod on the same URL `/playtest/cliffside/`. pck 35,332,972 B sha256 `80a4f8bc…a34fc`; wasm (37,700,666 B) and index.js byte-identical to web6, so only index.pck + the two index.html files changed. License fence clean (six self-authored kits_v8; overlay + pck grep both pass). Deploy truth: all routes 200, `index.wasm` application/wasm, live pck sha256 == local, headless-Chrome live boot booted=true 21.6 s with 0 page errors, joystick + CAST + keyboard all confirmed live. THE DUMMIES ARE NOT AT SPAWN — they are across the rope bridge to the north-east; captures include a driven route (run E 6 s -> NE 14 s -> N 6 s) showing all seven plus the relocated cow, and a Frozen Orb bolt in flight at them. Evidence + `deploy_truth.txt` in `reincarnated-collaboration/agentic_orchestration/drax/captures/2026-09-15-web-playtest-v7/`. Godot-side mirror committed local-only as `8702911`. vercel.json and the React bundle untouched; Vercel CLI still unauthenticated on this host, so no deployment ID recorded.)
**Prior update:** 2026-09-15 (**v1.18 /playtest Cliffside redeploys, three in sequence, all via git push to prod** (R-C3-110/120, gandalf RUN-CONDUCTOR). (1) `d07728b` CS-pack-web3 Frozen Orb v3 hybrid-spec, pck 34,844,732 B sha256 `cdb61eb0…c772`. (2) `40a3de3` CS-pack-web5 six-kit VFX bake-off (frozen_orb, blackwater_cocktail, poisonous_concoction, lightning_blast, zeus_chain, healing_hands), pck 35,113,536 B sha256 `05456bc3…74d1`. CS-pack-web4 was withdrawn upstream and never built. (3) **CURRENT PROD** `fd1e9ab` CS-pack-web6 (same six kits, tinted particles, softer flash), pck 35,123,448 B sha256 `34a949af…6695`. Each one: routes 200, live pck sha256 equals the local build, headless-Chrome live boot, and live cast-landing frames (one per kit for v5/v6) in `reincarnated-collaboration/agentic_orchestration/drax/captures/2026-09-15-web-playtest-v{3,5,6}/`. Landing `public/playtest/index.html` now reads "six-kit VFX bake-off — Tab / VFX button cycles them". vercel.json and the React bundle are untouched. Vercel deployment IDs were not recorded, because the CLI is absent or unauthenticated on this host.)
**Prior update:** 2026-09-15 (**v1.17 /playtest: Cliffside Godot 4.6.3 web build, DEPLOYED TO PROD via git push.** Matt 2026-09-15 "load this to the vercel app so friends can play test it on their phones" (gandalf dispatch, Run C-3). Static files only: `public/playtest/index.html` (landing) + `public/playtest/cliffside/` (Godot Web export, threads OFF, pck 34.84 MB, wasm 37.7 MB). Commit `ef5e544` pushed `fb1e5ec..ef5e544`, which auto-deployed Vercel Production deployment 6450649000 (success, `reincarnated-loadout-4kfr8k5ot`). Deploy truth: `/playtest`, `/playtest/`, `/playtest/cliffside` and `/playtest/cliffside/` all 200 (static wins over the SPA rewrite; vercel.json UNTOUCHED), `index.wasm` application/wasm (brotli), `index.pck` sha256 equals the local build, headless-Chrome live boot + touch + cast screenshots in `reincarnated-collaboration/agentic_orchestration/drax/captures/2026-09-15-web-playtest/`. Landing at 375px: no horizontal scroll. React bundle unchanged (`index-BcpWTLlg.js`). Build pipeline + source live in reincarnated-godot `web/` (local commit `60c1063`). NOTE: the Vercel CLI token on this host is INVALID (`vercel whoami` fails), so prebuilt CLI deploys need a `vercel login` first; git-push auto-deploy works.)
...
## Atlas D6-b v2 — legend anchor bottom-LEFT → bottom-RIGHT (footer-honesty occlusion fix) (2026-07-15)

**Spec:** `reincarnated-collaboration/agentic_orchestration/gandalf/notes/2026-07-15-atlas-interactive-glance-spec.md` **§9.6 D6-b v2** (line 271) + acc 67 v2 (line 278).
**Authority:** SURGICAL FIX charge from gandalf (SPEC-AUTHOR), caught at the gandalf verify gate. The D6 build's bottom-LEFT legend passed the v1 corner-enumeration probe (title/poles/key/dots) yet OCCLUDED the leading words of the in-artifact footer honesty block (feasible-kit denominators at canvas x=96, lit/unmapped/sealed counts, emission-provenance line). The law is upgraded: corner enumeration protected a LIST; the invariant is a CLASS. Auto-commit per CLAUDE.md team discipline. **NO deploy — promotion is a separate gandalf-gated step; PREVIEW alias untouched.**

**THE LAW (v2, authoritative):** the legend overlay may intersect the screen-space bbox of NO in-artifact `<text>` node (±4px pad) and no drill-in dot. ALL text binding (title, poles, condensation key, footer honesty block, points-denominator line, GHOST FIELD + graveyard annotations, all current + future). NON-binding for the translucent panel: ghost-lattice speckle + dashed boundary curves (text destroyed under a panel; a dashed-curve segment stays inferable — that asymmetry is the rationale).

**Scope held (frozen everything else):** ZERO diffs to the table (`AtlasBuildTable.tsx`), filter (`atlasPivot.ts`), column (`atlasColumns.ts`), vendored `public/atlas/` SVGs+JSON, and the write-set/viewBox math (`atlasLens.ts`, `useAtlasStage.ts`). Verified `git diff HEAD --name-only`: only 3 files changed.

**Files changed (3):**
- `src/components/atlas/AtlasLegend.tsx` — wrapper anchor `bottom-2 left-2` → `bottom-2 right-2` + `justify-end` (right-anchored, grows leftward). TWO load-bearing width/position values, both derived from measured `<text>` screen bboxes (probe), NOT viewport-tuned magic px: (1) `sm:max-w-[14.5%]` (of region, replaces fixed `sm:max-w-[240px]`) — the points-denominator line's tail reaches ≈84.1% of region width; a right-anchored panel clears it (±4px) iff width ≤ ≈14.9% region. A FIXED px cap clipped the tail at 1280 (fixed panel = bigger fraction of the narrower region); a FRACTIONAL cap clears it at every desktop width. (2) mobile `bottom-[23%]` (desktop reverts `sm:bottom-2`) — buttons carry a 44px min-height touch-target floor (globals.css @media max-width:640px, a11y invariant, KEPT/not shrunk); at 375 that 44px chip is taller than the ≈35px clear strip below the footer honesty band, and just above the band the interior is dense with drill-in dot labels, so the chip fits ONLY in a narrow window (chip-bottom ≈396–401 display-px) — 23% region-height centers it (≈6px margin each side).
- `scripts/atlas/d6-verify-probe.mjs` — ADDED the v2 assertion: for EVERY svg `<text>` node with nonzero bbox, compute screen-bbox intersection with the legend overlay rect (±4px pad); collect hits with content. Runs at 1440 (dark + light-flip) + 1280 (dark) + 375 (dark + light-flip). NEW top-level FAIL-LOUD gate (exit 1) if any checked surface has textOcclusions>0, printing the offending `<text>` content; REQUIRED surfaces = {1440,375}×{dark,light}. The light-skin re-check lives in the skin-flip block (re-finds the legend + re-scans text post-flip). Corner crops extended to add `bottomright` (new legend home).
- `src/pages/Atlas.tsx` — comments only: the D6 header block + the in-JSX legend comment updated bottom-left → bottom-right + the v2 law (the anchor lives in AtlasLegend, no class change here).

**Receipts (built dist via `npx vite preview` @4319, CDP against system Chrome — `*.vercel.app` SSO-walls headless CDP):**
- **acc 67 v2 — textOcclusions=0 at ALL checked surfaces:** `1440-dark` legend[l=1194,t=972,r=1392,b=1150] · `1440-light` legend[l=1194,t=317,r=1392,b=495] · `1280-dark` legend[l=1056,t=852,r=1232,b=1030] · `375-dark` + `375-light` legend[l=264,t=353,r=335,b=397]. Probe prints **"acc 67 v2 PASS"** (exit 0). textNodes=135 scanned per surface.
- **Corner crops (eyes-ref):** bottom-RIGHT shows the panel over ghost-lattice speckle + a dashed-curve segment (both NON-binding, permitted) with the points-denominator line + GHOST FIELD annotations fully visible to its LEFT — NOT occluded. bottom-LEFT shows the footer honesty block (193/11,160 line, 469-active line, 767,411,820-feasible line, register-v1.2 emission line, GRAVEYARD † ledger) FULLY VISIBLE (the v1 defect surfaces, now un-occluded). Mobile 375: chip lifted above the footer band, clears the dot-label band above + honesty strip below.
- **No-regression:** corner=right-bottom + inRegionDom + geomInside at all VPs; coverage 14.4%/14.5%/21.7% (all <25%); no normal-flow band; frame==fitbox-inset-12u + plate==viewBox match (both skins); subtitle "Edition-II lattice · …" (single Edition-II); wiring mark→selection+halo+0.75px stroke intact.
- **Build:** `npm run build` clean (0 TS errors; pre-existing chunk-size warning). Arbitrary Tailwind values confirmed in built CSS (`bottom:23%`, `max-width:14.5%` — literal class strings, JIT-generated, not purged).

**Verification:** **189/189 tests green** (12 files, unchanged count — no test asserted the old bottom-left anchor; the only legend tests are `LEGEND_ENTRIES` vocabulary checks, anchor-agnostic). Frozen-file guard PASS. Full probe green.

**Judgment calls (for gandalf/galadriel eyes-verify):**
1. Desktop panel width cap changed fixed `sm:max-w-[240px]` → fractional `sm:max-w-[14.5%]` — required to clear the points-line tail at 1280 (a fixed px cap clipped it there). This is the task-sanctioned "adjust width to clear the text node's bbox," NOT a HALT-dodging silent shrink; 14.5% fits the panel's natural content at 1440 with no squeeze.
2. Mobile chip lifted to `bottom-[23%]` (above the footer honesty band) rather than hugging the extreme bottom-right corner — FORCED by the 44px a11y touch-target floor being taller than the ≈35px clear strip below the footer. It's the one lawful bottom-right seat at 375; full touch size preserved (no shrink/drop/scroll).
3. **KNOWN EDGE (flagged, not a defect at the spec's viewports):** the mobile lawful window is narrow because a FIXED 44px chip must fit between two SCALING text bands (dot labels above, footer below). Below ≈360px viewport width the region shrinks enough that the window can close. 375 (the spec's mobile viewport) is clean. The honest sub-360 fix, if ever required, is a collapse-to-ICON (no text word) that drops under the dot-label band — NOT a further nudge. // TODO(drax): if a <360px guarantee is later required, collapse the mobile chip to an icon-only affordance.
