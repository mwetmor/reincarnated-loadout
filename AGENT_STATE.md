...
**Last updated:** 2026-07-15 (atlas **D5** — the "screen box" resizes to the D4 fit mount; §9.5; acc 63–65 green; PLATE + svg-sizing folded into the mount write-set; table/filter/column ZERO diffs; Vercel PREVIEW shipped, prod HELD for gandalf verify)
**Last commit:** `<pending>` atlas D5 — the mount-time write becomes a WRITE-SET (viewBox · planeClip · PLATE · svg-sizing) all to the derived fit box: the canvas plate rect follows the fit box (STRUCTURAL + FAIL-LOUD identification — direct-child rect of svg matching native dims + fill; 0/>1 → throw) so every mark/rail/footer sits ON the plate + no page-bg band inside the frame; the svg's width/height attrs are removed so CSS `w-full h-auto` + the viewBox 4:3 govern rendered size (responsive, zero h-overflow). D4's fit MOUNT is RATIFIED + untouched. On top of D4 (3aa1d66).
**Prior commit:** `3aa1d66` atlas D4 correction — STATIC full-horizon map: mounts at the DERIVED FIT BOX written ONCE to viewBox + planeClip; overflow-auto scroll stage → w-full h-auto page flow; scroll-nav math retired; §9.3 verbatim law amended per §9.4. On top of D3 (0a09713).
